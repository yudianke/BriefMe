...
排序以**事件重要性**为主，来源多样性只作修正项（差 3 家以上才可越过重要性），
其次看报道篇数与时效性。修正强度见 DIVERSITY_W。
"""
...
# 排序时每多一家不同媒体，等效前移的名次数。
# 取 0.4 是算出来的，不是拍的：重排公式是
#   score = (TOP_N - model_rank) + DIVERSITY_W * (n_sources - 1)
# 设 A 比 B 重要一名、B 比 A 多 d 家来源，则 B 反超 A 当且仅当 W*d > 1，即 d > 1/W。
#   W=0.4  -> d > 2.5，差 3 家才反超，**差 1-2 家一律按重要性排**（当前口径）
#   W=1.5  -> d > 0.67，差 1 家就反超（旧口径）
# 旧口径实测排出过「肯尼亚旅游直升机坠毁(10家) 压过 以色列承认打死 5 岁女孩(8家)」，
# 多样性只差 2 家却盖过了明显的重要性差距，所以改成现在这个。
DIVERSITY_W = 0.4
...
_SYSTEM = (
    ...
    "\n"
    "第二步：排序。**以事件本身的重要性为主**，来源多样性只作修正项：\n"
    "  - 先按事件重要性排；\n"
    "  - 再看来源多样性（报道该事件的**不同媒体家数**）：只有当某事件比另一事件\n"
    "    **多 3 家以上**不同媒体报道时，才可以越过重要性排到前面；\n"
    "    家数差距在 2 家以内的，一律按重要性排，不要因为多一两家就往前挪。\n"
    "  - 同等条件下再看报道总篇数与时效性。\n"
    "  来自 4 家不同媒体的事件，仍优先于同一家媒体发了 4 篇的事件。\n"
    "\n"
